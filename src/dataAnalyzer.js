...
const calculateWeeksData = (numOfWeeks, data)=>{
    ...
    for(const weekData of data)
    {
        ...
        if(weekData.high > highest)
            highest = weekData.high;
        if(weekData.low < lowest)
            lowest = weekData.low;
        i++;
    }
    ret['weeks'] = i;
...
const analyzeSymbol = async (symbol) =>
{
    ...
    weeksData = Object.values(weeksData['Weekly Time Series']).map(value => {return{'open': Number(value['1. open']),
